...
@implementation RedHotCoolProjectObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_redhotcoolproject);
	RELEASE_TO_NIL(modules);
...
-(void)injectPatches
{
	// called to inject any RedHotCoolProject patches in JS before a context is loaded... nice for 
...
{
	// create RedHotCoolProject global object
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_redhotcoolproject = [[RedHotCoolProjectObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_redhotcoolproject];
	
	NSString *_redhotcoolprojectNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _redhotcoolprojectNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_redhotcoolproject valueForKey:name];
...
-(id)require:(KrollContext*)kroll path:(NSString*)path
{
	...
	@throw [NSException exceptionWithName:@"org.redhotcoolproject.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
